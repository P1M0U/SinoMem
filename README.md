...
Agent 框架（如 Hermes）通常自带会话级记忆——`session_search`、`user_profile` 等工具处理**当前会话内的上下文**。本项目解决的是另一个层次的问题：
...
本项目的独特价值在于**不绑定任何 Agent 框架**。同一份 `.db` 文件可以被多个 MCP 兼容的 Agent（Hermes、Claude Code、Cursor 等）共享：

```
┌─────────┐  ┌──────────┐  ┌──────────┐
│ Hermes  │  │ Claude   │  │ Cursor   │  ... 任意 MCP 兼容 Agent
└────┬────┘  └────┬─────┘  └────┬─────┘
     │            │             │
     └────────────┼─────────────┘
                  │ MCP 协议 (stdio)
...
这意味着：
- 用 Hermes 记下的事，切换到 Claude Code 后仍然可查
- 一个 Agent 学会的用户偏好，另一个 Agent 也能利用
- 记忆数据独立于框架版本，不会因升级丢失
- 一份记忆在多个工具间形成真正的"长期知识库"

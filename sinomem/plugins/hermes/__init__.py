...
1. Hermes 原生接口 — SinoMemProvider（实现 MemoryProvider）
2. BasePlugin Python API — HermesPlugin（统一插件接口）

快速使用：
    # 方式 1: Hermes 原生（由 plugin.yaml 自动加载）
    from sinomem.plugins.hermes import SinoMemProvider
...
from .provider import SinoMemProvider
...
class HermesPlugin(BasePlugin):
    ...
    底层 engine 创建由 MemoryEngine 处理，与 SinoMemProvider 共享数据库。
    """
...
__all__ = ["SinoMemProvider", "HermesPlugin"]
